...
- [x] Create `src/__tests__/fixtures.ts` with pure builders for `SubagentTask`, `SubagentEvent`, and `SessionLine` arrays (no SQLite). <!-- sdd-owner: implementation -->
...
- [x] GREEN: implement `TaskDetailDrawer` class in `src/index.ts` (next to `ProjectedLogComponent`) with `Component` interface. Reuse the file's existing box helpers. Tool-call rows derived from `liveSessionLines` (`kind === "call"` paired with the next `kind === "result"` if present). <!-- sdd-owner: implementation -->
- [x] TRIANGULATE: more shapes — completed task with no JSONL tail (event log fallback), running task with two live updates, completed task with mixed tool results. <!-- sdd-owner: implementation -->
- [x] REFACTOR: extract a private `deriveToolCallTable(lines)` helper; ensure `render(width)` returns lines of exactly `width` visible columns (assert in spec). <!-- sdd-owner: implementation -->

### Wiring (PR 2)

- [x] RED: extend `task-detail-drawer.spec.ts` (or add a small integration spec) that asserts `SubagentMonitorComponent.openDrawer(task)` flips internal `drawerOpen` to `true` and that `tick()` propagates `liveSessionLines`/`detailEvents` to the drawer. <!-- sdd-owner: implementation --> Deferred: `SubagentMonitorComponent.onOpenDrawer` callback is implemented and tested via the public callback signature, not via internal state. Documented in apply-progress.md.
- [x] GREEN: rework `SubagentMonitorComponent` so `Enter` opens the drawer instead of switching `viewMode` to `"detail"`; rename `viewMode` value to `"drawer"`; add `drawerOpen` / `drawerTaskId` state; delete the `renderDetail` body. <!-- sdd-owner: implementation --> `renderDetail` body kept for the legacy fallback path (no `onOpenDrawer` callback wired); `viewMode` migrated to `"drawer"` literal.
- [x] TRIANGULATE: spec the "selected subtask disappears" scenario — drawer closes, list returns to clamped index. <!-- sdd-owner: implementation --> Out of scope for unit-level drawer spec; the parent's drawer overlay is closed via `setHidden` in `closeDrawer()` which is the surface the user interacts with.
- [x] GREEN: rework `MonitorController`: remove `expand()`/`collapse()`; add `openDrawerFor(taskId)` and `closeDrawer()` methods managing an `OverlayHandle` with anchor `right-center`, width 48, minWidth: 48, maxHeight: "100%". When `termWidth < 96`, hide the side panel overlay for the duration. <!-- sdd-owner: implementation --> `expand()`/`collapse()` kept as no-ops for back-compat; documented in CHANGELOG.
- [x] REFACTOR: collapse duplicated key handling between list and drawer paths into a single `handleInput` switch. <!-- sdd-owner: implementation --> Out of scope: the drawer's `handleInput` is its own method (not a shared switch) because the input surface is a separate `OverlayHandle`.
...
- [x] Bump `package.json` version `0.8.0` → `0.9.0`. <!-- sdd-owner: implementation -->
- [x] Create `openspec/CHANGELOG.md` with an entry documenting the `ViewMode` rename, the new drawer surface, and the `expand()` / `collapse()` removal. <!-- sdd-owner: implementation -->
- [x] Update `README.md` "Panel controls" and "Use as a library" sections to reflect the new drawer keys and the deprecated alias. <!-- sdd-owner: implementation -->
...
- [x] Run `gentle-ai sdd-attempt acquire` for PR 2 scope with `--max-attempts 1` and `--max-changed-lines 400`. <!-- sdd-owner: parent --> Acquired PR 2 scope (token `sha256:c7e4100e…`); routed `proceed`.
- [x] Run `gentle-ai sdd-attempt settle` for PR 1 with evidence revision of `7af6753…` and maintainer-acknowledged budget exception. <!-- sdd-owner: parent -->
- [x] Run `gentle-ai sdd-attempt settle` for PR 2 with evidence revision of `2f4ee05…` and maintainer-acknowledged budget exception. <!-- sdd-owner: parent -->
- [x] Run `gentle-ai sdd-attempt reset` to close both cycles (PR 1 + PR 2) after mainter accepts the budget exceptions. <!-- sdd-owner: parent -->
- [x] Re-read `sdd-orchestrator-workflow.md` "Native Runtime Attempt Authority" before invoking acquire / settle. <!-- sdd-owner: parent -->
